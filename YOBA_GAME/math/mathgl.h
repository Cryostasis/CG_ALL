...
inline const mat4 GLrotate(float x, float y, float z)
{
	return GLFromEuler(x * 180.0 / M_PI, y * 180.0 / M_PI, z * 180.0 / M_PI);
}

inline const mat4 GLrotate(const vec3 &r)
{
	return GLrotate(r.x, r.y, r.z);
}
...
inline const vec3 GLToEulerRad(const mat4 &M)
{
	vec3 res = GLToEuler(M);
	res = res * M_PI / 180;
	return res;
}
